process run_nsforest_process {
    ...
    input:
        tuple path(h5ad_file), val(label_key), val(embedding_key), val(organism),val(disease),
	      ...
	      path(medians_h5ad_file), path(binary_scores_h5ad_file)
    ...
    script:
    """
    nsforest-cli run-nsforest --input-path $binary_scores_h5ad_file --cluster-header $label_key --output-folder nsforest-results-$label_key-$embedding_key-$h5ad_file
    """
}
